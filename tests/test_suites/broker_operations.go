...
func BrokerOperationsSuite(t *testing.T, svcs []*brokerClient.Service, sbc *brokerClient.ServiceBrokerClient, async bool) {
	...
	for _, svc := range svcs {
		...
		if async == false && res.StatusCode != http.StatusCreated {
			t.Fatal(fmt.Sprintf("Unexpected http status code when creating %s synchronously", svc.Name), res.StatusCode, err.Description)
		}
		...
		if async == true {
			...
			// Poll for successful completion of Provision using LastOperation
			wg := sync.WaitGroup{}
			wg.Add(1)
			go func(){
				defer wg.Done()
				...
				for hasFinished == false {
					...
					// Ignore errors/error states as LastOperation can return errors/error states as operations are initialising.
